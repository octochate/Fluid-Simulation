...
void advect(int N, int b, float *d, float *d0, float *u, float *v, float dt)
{
	...
	if (square == 1 && i >= 32 && i < 98 && j >= 32 && j < 98)
	{
		d[IX(i, j)] = 0.0;
		d0[IX(i, j)] = 0.0;
	}
...
static void Sliders()
{
	...
	glutBitmapString(GLUT_BITMAP_8_BY_13, "Fluid Size");
	glRasterPos2f(0.64453125, 0.7500000);
	glutBitmapString(GLUT_BITMAP_8_BY_13, "Fluid Amount");
	glRasterPos2f(0.64453125, 0.7109375);
	glutBitmapString(GLUT_BITMAP_8_BY_13, "Force Size");
	glRasterPos2f(0.64453125, 0.6718750);
	glutBitmapString(GLUT_BITMAP_8_BY_13, "Force Amount");
	...
	float vortMax = 1.;
	float vortMin = 0.;
	float fluidSizeMax = 1.;
	float fluidSizeMin = 0.;
	float fluidAmountMax = 1.;
	float fluidAmountMin = 0.;
	float forceSizeMax = 1.;
	float forceSizeMin = 0.;
	float forceAmountMax = 1.;
	float forceAmountMin = 0.;
	...
	for (i = 0; i <= N; i++)
	{
		...
		for (j = 0; j <= N; j++)
		{
			...
			if (square == 1 && i >= 34 && i < 62 && j >= 34 && j < 62)
			{
				...
				d10 = 1.0;
				d11 = 1.0;
				glColor3f(0.0, d00, 0.0);
				glVertex2f(x, y);
				glColor3f(0.0, d10, 0.0);
				glVertex2f(x + h, y);
				glColor3f(0.0, d11, 0.0);
				glVertex2f(x + h, y + h);
				glColor3f(0.0, d01, 0.0);
				glVertex2f(x, y + h);
			}
			else
			{
