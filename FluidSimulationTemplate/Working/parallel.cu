...
int main(int argc, char* argv[]) {
    ...
    // diffuse
    float a = dt * visc * N * N;
    lin_solve << < num_blocks, num_threads >> > (N, 1, u_cuda, u_prev_cuda, a, 1 + 4 * a); // diffuse u_cuda
    lin_solve << < num_blocks, num_threads >> > (N, 2, v_cuda, v_prev_cuda, a, 1 + 4 * a); // diffuse v_cuda
    a = dt * diff * N * N;
    lin_solve << < num_blocks, num_threads >> > (N, 0, dens_cuda, dens_prev_cuda, a, 1 + 4 * a); // diffuse dens_cuda
    cudaDeviceSynchronize();
