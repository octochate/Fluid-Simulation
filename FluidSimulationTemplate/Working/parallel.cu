...
__device__ void add_source(int N, float* x, float* s, float dt, int idx, int size)
{
    if (idx < size) {
        x[idx] += dt * s[idx];
    }
...
__global__ void vel_step_parallelize(int N, float *u_cuda, float *v_cuda, float *u_prev_cuda,float * v_prev_cuda,int visc, int dt, int size) {
    int idx = threadIdx.x;
    add_source(N, u_cuda, u_prev_cuda, dt, idx, size);
    add_source(N, v_cuda, v_prev_cuda, dt, idx, size);
...
__global__ void den_step_parallelize(int N, float *dens_cuda, float* dens_prev_cuda, float *u_cuda,float* v_cuda, int diff, int dt, int size) {
    int idx = threadIdx.x;
    add_source(N, dens_cuda, dens_prev_cuda, dt, idx, size);
...
int main(int argc, char* argv[]) {
    ...
    vel_step_parallelize << <num_blocks, num_threads >>> (N, u_cuda, v_cuda, u_prev_cuda, v_prev_cuda, visc, dt, size);
    cudaDeviceSynchronize();

    // Density timestep parallelization
    den_step_parallelize << <num_blocks, num_threads >> > (N, dens_cuda, dens_prev_cuda, u_cuda, v_cuda, diff, dt, size);
    cudaDeviceSynchronize();
