...
volatile int dvel; // used to toggle displays between velocity and density
...
void normalizeMatrix(f3 f[NUM_ROW][NUM_COL], float scalingFactor) {
    ...
    for (int x = 0; x < NUM_COL; x++) {
        for (int y = 0; y < NUM_ROW; y++) {
            if (f[y][x].x != 0) f[y][x].x = f[y][x].x / abs(f[y][x].x) * scalingFactor;
            if (f[y][x].y != 0) f[y][x].y = f[y][x].y / abs(f[y][x].y) * scalingFactor;
            f[y][x].cap(0.1);
        }
    }
